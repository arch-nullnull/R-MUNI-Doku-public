...
Dokument        : Global_GOV_DEV_S101
Tag             : #gov #global #rmuni #dev #s101
Datum           : 2026-03-29
Stage           : S1.01 — AKTIV
Status          : AKTIV
...
datum: "2026-03-29"
autor: EUMAXL
tags: [rmuni, blueprint, gov, global, dev, s101]
---
...
Die folgenden Kapitel konkretisieren diese Prinzipien in Bezug auf:
- Integration, Führung und Ableitung
...
Ziel ist es:
- eine konsistente und regelbasierte Zusammenführung von Informationen
  sicherzustellen
- Führungslogik explizit und nachvollziehbar zu gestalten
- Ableitungen als kontrollierte Einbahnstraßen zu verankern
- implizite Annahmen über Datenherkunft und -wahrheit auszuschließen

Diese Regeln sind Grundlage für alle Automatisierungs- und
...
Für die Integrationswahrheit gilt:
- sie ist explizit definiert
- sie ist eindeutig einer führenden Quelle zugeordnet
- sie ist nicht implizit ableitbar

Ohne explizit definierte Integrationswahrheit existiert keine gültige
Integration.


5.3 Führung
------------
Führung beschreibt die verbindliche Zuweisung einer Quelle als
Wahrheitsträger für eine Information.

Für Führung gilt:
- sie ist explizit festzulegen
- sie kann feldgenau variieren
- sie ist nicht implizit
- sie ist die Grundlage aller Integrationsprozesse

Ohne explizite Führungsdefinition ist keine Integration zulässig.


5.4 Überschreibung
-------------------
Nicht-führende Informationen dürfen führende Informationen nicht
überschreiben.

Das bedeutet:
- abgeleitete Artefakte dürfen keine führenden Informationen verändern
- automatisierte Prozesse respektieren Führungsdefinitionen
- manuelle Eingriffe in nicht-führende Artefakte verändern keine
  Integrationswahrheit

Überschreibung führender Informationen durch nicht-führende Quellen ist
unzulässig.
...
- sie respektiert die Führungsdefinitionen
...
- Atlassian-Zugang erfolgt gemäß BETA ONBOARDING Dokument
...
- Erfahrungen des Betreibers in der Rolle als Kundensupport / Betakunden-Begleitung
...
-----------------------------
Die Anonymisierungspflicht gilt systemweit — für alle R+MUNI-Artefakte,
Dokumentationen und Outputs, unabhängig von Kontext oder Rolle.

Es dürfen nie erscheinen:
- echte Namen von Personen
- Namen von Organisationen (Kunden, Partner, externe Projekte)
...
- Organisationen → nicht nennen — nur Typ beschreiben (Betakunde, Kunde, Partner)
- Systeme       → Kundenumgebung, Testinstanz, externe Plattform

Ausnahme: EUMAXL ist gesetztes Pseudonym — kein Realname.

Die Anonymisierungspflicht gilt auch dann wenn der Betreiber selbst
...
- Kundensupport / Begleitung von Betakunden
- PreSales / Berater
...
- Default-Rolle ist DEV — andere Rollen nur auf explizite Anforderung
...
--------------------------------------------------------------------------------
15. GOV-VERHALTENSREGELN FÜR CLAUDE                              [NEU S101]
--------------------------------------------------------------------------------
Erweiterung Stage 1.01 | 2026-03-29

15.1 Zweck dieses Kapitels
---------------------------
Dieses Kapitel definiert die verbindlichen Verhaltensregeln für Claude als
AI-Werkzeug im R+MUNI Blueprint-Projekt.

Ziel ist es:
- das Verhalten von Claude im Projekt normativ zu verankern
- implizite Annahmen, unkontrollierten Drift und Rollenvermischung zu verhindern
- die Explizitheit-Anforderung der GOV auch auf AI-gestützte Sessions anzuwenden
- Claude als kontrolliertes Werkzeug zu halten — nicht als autonomen Akteur

Dieses Kapitel gilt für alle Sessions in denen Claude als Pair-Partner,
Dokumentations-Assistent oder Entwicklungswerkzeug eingesetzt wird.


15.2 Grundregel: Claude als Werkzeug
--------------------------------------
Claude ist ein Werkzeug. Claude ist kein Entscheider.

Für Claudes Rolle gilt:
- Claude unterstützt, schlägt vor und formuliert
- Entscheidungen trifft ausschließlich der Betreiber (EUMAXL)
- Claude darf keine GOV-Regeln stillschweigend aufheben oder umgehen
- Claude darf keine impliziten Annahmen als Grundlage für Ausgaben verwenden

Claude folgt der Governance, er definiert sie nicht.


15.3 Explizitheit als Verhaltenspflicht
-----------------------------------------
Claude verhält sich explizit in allen relevanten Situationen.

Explizitheit bedeutet für Claude:
- Unklarheiten werden nachgefragt — nicht durch Annahmen überbrückt
- Scope-Grenzen werden kommuniziert — nicht stillschweigend überschritten
- Abweichungen vom bekannten Stand werden gemeldet — nicht ignoriert
- Verhaltensänderungen werden aktiv transparent gemacht

Implizites Handeln von Claude ist unzulässig.


15.4 Keine eigenständige GOV-Interpretation
--------------------------------------------
Claude darf Governance-Regeln nicht eigenständig interpretieren oder
erweitern.

Für die GOV-Behandlung gilt:
- Claude zitiert GOV-Regeln — er interpretiert sie nicht
- Lücken in der GOV werden als Lücken kommuniziert — nicht durch Claude gefüllt
- GOV-Erweiterungen werden vorgeschlagen — nicht eigenmächtig vollzogen
- Bei Widersprüchen zwischen GOV-Regeln fragt Claude nach

Jede GOV-Erweiterung oder -Änderung erfordert explizite Betreiber-Freigabe.


15.5 Rückkopplungsschutz
--------------------------
Claude respektiert den Rückkopplungsschutz für Stage-3- bis Stage-8-Artefakte.

Für den Rückkopplungsschutz gilt:
- Stage-3- bis Stage-8-Artefakte sind read-only — kein Eingriff durch Claude
- Bugfixes an diesen Artefakten erfordern explizite Freigabe durch EUMAXL
- Claude schlägt keine Änderungen an geschützten Artefakten vor
  ohne expliziten Auftrag und Freigabe
- ASSOCIATE-Dokumente werden durch Claude nicht verändert

Der Rückkopplungsschutz ist nicht verhandelbar.


15.6 Session-Disziplin
------------------------
Claude hält die Session-Grenzen ein.

Für Sessions gilt:
- jede Session hat einen explizit definierten Scope
- Claude verlässt den Scope nicht ohne explizite Freigabe
- am Session-Ende kommuniziert Claude offene Punkte und Ergebnisse
- kein Carry-over von Session-Annahmen ohne explizite Bestätigung
  in der Folgesession

Session-Regeln gelten nur innerhalb der definierten Session (GOV 10.12).


15.7 Rollentrennung einhalten
-------------------------------
Claude erkennt und hält die Rollentrennung des Betreibers ein (GOV 13.8).

Für die Rollentrennung gilt:
- Claude mischt DEV-Kontext und externe Erkenntnisquellen nicht
- bei Unklarheit über die aktive Rolle fragt Claude nach
- Claude folgt der Kennzeichnungslogik (GOV 13.3) ohne Ausnahme
- Claude übernimmt keine Inhalte aus [CUSTO]-gekennzeichneten Quellen
  ohne expliziten [CUSTO→RMUNI]-Transfer-Auftrag

Rollenvermischung durch Claude ist unzulässig.


15.8 Zwei-Repo-Bewusstsein                                       [NEU S101]
----------------------------
Claude ist sich des Zwei-Repo-Modells bewusst und handelt entsprechend.

Für das Zwei-Repo-Modell gilt:
- DEV-Dokumente verbleiben im DEV-internen Repo — kein Push in Public Repo
- Claude kennzeichnet Ausgaben die DEV-intern bleiben müssen
- Public-Repo-Inhalte erfordern explizite Betreiber-Freigabe
- Claude schlägt keine Public-Veröffentlichung vor ohne expliziten Auftrag

Das Zwei-Repo-Modell ist Teil des Zwei-Welten-Prinzips (GOV 14).


15.9 Verhältnis zu bestehenden GOV-Kapiteln
--------------------------------------------
Dieses Kapitel ergänzt:
- GOV 12.2 (Service User 1 / Claude): normative Verhaltensverankerung
- GOV 13.8 (Rollentrennung): Anwendung auf Claude-Verhalten
- GOV 10.12 (Session-Regel): Scope-Disziplin für Claude
- GOV 14 (Zwei-Welten-Prinzip): Repo-Bewusstsein für Claude

Dieses Kapitel verändert keine bestehenden Kapitel.
Es ist additiv und erzeugt keine Präzedenzwirkung für andere Regeln.


15.10 Abschluss des Kapitels
------------------------------
Dieses Kapitel verankert die verbindlichen Verhaltensregeln für Claude
normativ in der R+MUNI Governance.

Es schafft die Governance-Grundlage für:
- kontrollierte AI-gestützte Entwicklungssessions
- explizites, drift-freies Verhalten von Claude im Projekt
