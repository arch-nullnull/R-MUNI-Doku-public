...
Dokument        : NBX_FLOW_How2_S105
Tag             : #dev #how2 #nbxflow #s105
Datum           : 2026-04-14
Stage           : S105 — AKTIV
Status          : AKTIV
...
Jira-Sync       : NEIN
Erstellt        : 2026-04-06
Letzte Änderung : 2026-04-14 — S105-Update | Basis: NBX_How2_DEV_S102.md
Ablageort       : 02-how2\NBX_FLOW_How2_S105.md
================================================================================


VORAUSSETZUNGEN
--------------------------------------------------------------------------------
- [[NBX_FLOW_principles_S105]] gelesen — Konzept und Designentscheidungen bekannt
- Python 3.10+
...
- Empfohlen: PowerShell als Administrator für vollständige ARP-Scans
...
NBX02 – Netzwerk-Scan  ★ LAUFZEIT BIS 5 MINUTEN
  ...
  → Hinweis: 00-archimatearchive\ liegt in .gitignore

NBX03 – Normierung und CSV-Export
  py .\NBX03-normalize_to_csv.py
  → nbx_raw.json lesen, Host- und Service-Zeilen erzeugen
...
NBX04 – IP-Merge  ★ NEU S105
  py .\NBX04-ip_merge.py
  → Liest trash_nbx.csv aus 00-archimatechild\
  → Durchlauf 1: alle Host-Zeilen einsammeln
  → Durchlauf 2: Service-Zeilen dem jeweiligen Host zuordnen
  → Ergebnis: eine Zeile pro Host, Services aggregiert
  → Ziel: 01-artifacts\02-csv\03-child\00-archimatechild\trash_nbx.csv (überschrieben)

NBX05 – Übergabe-Report
  py .\NBX05-handoff_report.py
  → trash_nbx.csv und properties_nbx.csv auswerten
  → Statistik und nächste Schritte ausgeben
  → Nächster Schritt explizit: ECM00 → ECM01 (Phase 1) oder ECM00 → ECM02 (Phase 2)
  → Ziel: 02-stages\99-logs\NBX05-handoff_report.txt
...
nbx_config.txt enthält keine sensiblen Daten — kein Token, kein Passwort.
Kein .gitignore-Eintrag erforderlich.

nbx_raw.json enthält Scan-Rohdaten (IP-Adressen, MAC-Adressen) — liegt
...
trash_nbx.csv — nach NBX04: eine Zeile pro Host
  Header: 3PartyID, nbx_objecttype, Name, Manufacturer, Status,
          Description, nbx_source, nbx_raw_id

  Beispiel:
  nbx_192_168_1_12, host, Gaming-PC, , up, Port 22/tcp | Port 445/tcp, scan_lokal, 192.168.1.12
...
Erster Lauf (Vollständig):
  py .\NBX00-validate_environment.py
  py .\NBX01-validate_config.py
  py .\NBX02-scan_network.py          ★ bis 5 Minuten Laufzeit
  py .\NBX03-normalize_to_csv.py
  py .\NBX04-ip_merge.py
  py .\NBX05-handoff_report.py
  → weiter mit ECM00 → ECM01 (Mapping-Modell erstellen)
...
Abweichungen:
  Erneuter Scan (Config unverändert):
    NBX02 → NBX03 → NBX04 → NBX05

  Config geändert, neu scannen:
    NBX01 → NBX02 → NBX03 → NBX04 → NBX05

  Nur Merge neu (nbx_raw.json vorhanden):
    NBX03 → NBX04 → NBX05


================================================================================
KRITISCHE REGEL — ÜBERGABEPFAD AN ECM
================================================================================

Die trash_nbx.csv muss in folgendem Verzeichnis abgelegt sein:
...
Warum ist das kritisch?
  ...
  Dieser Fehler trat im Produktivrun S105 auf. (Layer-8-Problem)
...
Fehler: Mapping basiert auf altem Stand (kein Fehler sichtbar)
  Ursache: trash_nbx.csv lag in 04-import\ statt 00-archimatechild\ beim ECM-Lauf
  Lösung:  Datei in richtigen Ordner legen, ECM-Lauf wiederholen
...
------------------------------------------------   --------------------------------
Ersten Lauf starten                                NBX00 → NBX01 → NBX02 → NBX03 → NBX04 → NBX05
Erneut scannen (Config unverändert)                NBX02 → NBX03 → NBX04 → NBX05
Config geändert, neu scannen                       NBX01 → NBX02 → NBX03 → NBX04 → NBX05
Nur Merge neu (nbx_raw.json vorhanden)             NBX03 → NBX04 → NBX05
Prüfen ob Umgebung noch stimmt                     NBX00
Scan-Ergebnis an ECM übergeben — Erstlauf          ECM00 → ECM01 → [Archi: Mapping-Modell]
Scan-Ergebnis an ECM übergeben — Folgelauf         ECM00 → ECM02 → [Archi] → ECM03
...
[[NBX_FLOW_principles_S105]]    Designentscheidungen und Hintergrund
[[ECM_FLOW_How2_S105]]          Consumer des NBX-Outputs (Phase 1 + 2)
[[ECM_FLOW_principles_S105]]    ECM Konzept
[[DEV_Sprint_NBX-ECM-RUN_S105]] Produktivrun — Erkenntnisse S105
[[CSV_FLOW_How2_S105]]          Ziel-Flow nach ECM → Archi Import
[[NBX_How2_DEV_S102]]           Vorgänger-Dokument (read-only)


================================================================================
NBX_FLOW_How2 | S105 | 2026-04-14 | R+MUNI Blueprint
================================================================================
