...
Dokument        : M2B_FLOW_How2_S105
Tag             : #dev #how2 #m2bflow #s105
Datum           : 2026-04-14
Stage           : S105 — AKTIV
Status          : AKTIV
...
Jira-Sync       : NEIN
Erstellt        : 2026-03-02
Letzte Änderung : 2026-04-14 — S105-Update | Basis: M2B_FLOW_How2_S8.md
Ablageort       : 02-how2/M2B_FLOW_How2_S105.md
================================================================================
...
Schritt 1: Root auflösen
  ...
  → Erzeugt: 02-stages/99-logs/M2B00-root.resolved.txt
...
Schritt 2: BusinessProcess Hüllen extrahieren
  ...
  → Erzeugt: 02-stages/01-bpmnarchive/<Prozessname>.bpmn
...
Schritt 3: Hüllen aktivieren
  ...
  → Ziel: 00-model/01-bpmn/00-bpmnactive/
  → Überschreibt NICHT wenn Datei bereits existiert
...
--- MANUELLE PHASE: Camunda Anreicherung ---
  Die active BPMN Dateien in 00-model/01-bpmn/00-bpmnactive/ können nun
...
Schritt 7: BPMN-ID zurückschreiben in Archi-Elemente
  python M2B06-writeback_bpmn_id.py
  → Liest alle active BPMN Dateien
  → Schreibt BPMN Process ID als Property "BPMN_ID" in zugehöriges
    Archi-Element in master.xml
  → Nur wenn BPMN_ID noch nicht gesetzt (immutable)
  → Match: direkter ID-Match, Fallback via Name
  → Erstellt master.xml.bak vor Schreiben

Schritt 8: Scope-Snapshot aktualisieren
  python M2B07-scope_snapshot.py
  → Scannt master.xml nach sourceSystem/sourceModel Kombinationen
  → Schreibt Snapshot-Block in 02-stages/run-scope.txt
...
01-artifacts/00-xml/01-mapping/M2Bmapping.txt
...
master.xml.bak ist veraltet
  Hinweis: M2B05 und M2B06 überschreiben master.xml.bak bei jedem Lauf.
...
M2B06: BPMN_ID wird nicht gesetzt
  Ursache 1: BPMN_ID ist bereits in master.xml gesetzt (immutable).
    Lösung: Kein Handlungsbedarf – BPMN_ID ist bewusst unveränderlich.
  Ursache 2: Kein Match gefunden (weder ID noch Name stimmt überein).
    Lösung: BPMN-Prozess-ID oder -Name mit Archi-Element abgleichen.
    M2B06 loggt alle Nicht-Matches mit Details.
...
7. M2B04 → M2B05 → M2B06 → M2B07 ausführen
8. master.xml enthält nun den angereicherten Prozess inkl. BPMN_ID Property


TYPISCHER ARBEITSABLAUF – BESTEHENDEN PROZESS AKTUALISIEREN
--------------------------------------------------------------------------------
1. BPMN-Datei in 00-model/01-bpmn/00-bpmnactive/ in Camunda öffnen
2. Änderungen vornehmen, speichern
3. M2B04 → M2B05 → M2B06 → M2B07 ausführen
   (M2B00–M2B03 sind nicht notwendig wenn keine neuen Prozesse entstehen)
4. master.xml enthält den aktualisierten Stand

Hinweis M2B06 bei Update: BPMN_ID ist immutable. Wenn die Property bereits
gesetzt ist (Erstanlage), ändert M2B06 nichts – das ist korrekt.


SCOPE-SNAPSHOT LESEN
--------------------------------------------------------------------------------
Nach M2B07 enthält run-scope.txt einen Block wie:

  # SNAPSHOT_START
  # ==========================================================
  # MASTER.XML SNAPSHOT - generated by M2B07-scope_snapshot.py
  # Last updated: 2026-04-14T...
  # ==========================================================
  # --- sourceSystem: archi ---
...
================================================================================
M2B_FLOW_How2_S105 | 2026-04-14 | R+MUNI Blueprint | Stage 1.05
================================================================================
