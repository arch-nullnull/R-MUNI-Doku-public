...
Dokument        : ATL_FLOW_How2_S105
Tag             : #dev #how2 #atlflow #atlassian #s105
Datum           : 2026-04-14
Stage           : S105 — AKTIV
Status          : AKTIV
...
Jira-Sync       : NEIN
Erstellt        : 2026-03-08
Letzte Änderung : 2026-04-14 — S105-Update | Pfade korrigiert (01-artifacts, 02-stages)
Ablageort       : 99-doku/02-how2/ATL_FLOW_How2_S105.md
================================================================================
...
- BLUEPRINT_ROOT korrekt in root.cfg eingetragen
- Ordnerstruktur gemäß structure.txt vorhanden
- 02-stages/run-scope.txt enthält ein aktives SOURCE=ATL Pair
- master.xml in 01-artifacts/00-xml/00-master/ vorhanden
...
================================================================================
  ...
  cd 01-artifacts/01-scripts/
  py ATL00-validate_atl_scope.py

Was passiert:
  → Root-Pfad aus root.cfg auflösen
  → 02-stages/run-scope.txt auf aktives SOURCE=ATL Pair prüfen
  → master.xml auf Existenz und Lesbarkeit prüfen
  → Ordner prüfen: 02-stages/00-archimatearchive/ + 02-stages/99-logs/
  → Schreibt: 02-stages/99-logs/ATL00-root.resolved.txt
  → Schreibt: 02-stages/99-logs/ATL00-validate_atl_scope.log
...
Bei Fehler:
  ...
  master.xml fehlt → Pfad in root.cfg prüfen, XML Flow ausführen
...
Was passiert:
  → 02-stages/99-logs/ATL00-root.resolved.txt lesen
  → 02-stages/run-scope.txt → SOURCE=ATL Modelle ermitteln
  ...
  → Schreibt: 02-stages/00-archimatearchive/atl_<layer>.csv
    (nur Layer die Objekte enthalten erzeugen eine Datei)
  → Schreibt: 02-stages/99-logs/ATL01-masterXml2AtlCsv.log

Erwartete Konsolenausgabe (Erfolg):
  [ATL01] OK | N Datei(en) -> 02-stages/00-archimatearchive/
...
Was passiert:
  → 02-stages/99-logs/ATL00-root.resolved.txt lesen
  → Alle atl_*.csv aus 02-stages/00-archimatearchive/ lesen
  ...
  → Schreibt: 02-stages/00-archimatearchive/jira_ea_import.csv
  → Schreibt: 02-stages/99-logs/ATL02-atlCsv2JiraCsv.log

Erwartete Konsolenausgabe (Erfolg):
  [ATL02] OK | N Zeilen -> 02-stages/00-archimatearchive/jira_ea_import.csv
...
3. Datei hochladen:
   02-stages/00-archimatearchive/jira_ea_import.csv
...
→ 02-stages/run-scope.txt öffnen
...
→ Pfad: 01-artifacts/00-xml/00-master/master.xml

FEHLER: Ausgabe-Ordner fehlt
→ 02-stages/00-archimatearchive/ manuell anlegen
...
Scripts:
  01-artifacts/01-scripts/ATL00-validate_atl_scope.py
  01-artifacts/01-scripts/ATL01-masterXml2AtlCsv.py
  01-artifacts/01-scripts/ATL02-atlCsv2JiraCsv.py

Temporäre Outputs (werden bei jedem Lauf überschrieben):
  02-stages/00-archimatearchive/atl_business.csv
  02-stages/00-archimatearchive/atl_technology.csv
  02-stages/00-archimatearchive/atl_<weitere layer bei Bedarf>.csv
  02-stages/00-archimatearchive/jira_ea_import.csv

Logs:
  02-stages/99-logs/ATL00-root.resolved.txt
  02-stages/99-logs/ATL00-validate_atl_scope.log
  02-stages/99-logs/ATL01-masterXml2AtlCsv.log
  02-stages/99-logs/ATL02-atlCsv2JiraCsv.log

Konfiguration:
  02-stages/run-scope.txt   (SOURCE=ATL Pair manuell eintragen)
