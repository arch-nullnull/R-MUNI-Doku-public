...
Dokument        : CSV_FLOW_How2_S105
Tag             : #dev #how2 #csvflow #s105
Datum           : 2026-04-14
Stage           : S105 — AKTIV
Status          : AKTIV
...
Ablageort       : 02-how2/CSV_FLOW_How2_S105.md
================================================================================
...
- <rootfolder> korrekt in root.cfg eingetragen
...
Schritt 0: Master CSVs leeren (manuell)
  Die Dateien in 01-artifacts/02-csv/00-master/ müssen vor einem sauberen
...
Schritt 1: Umgebung validieren
  ...
  → Prüft root.cfg, löst <rootfolder> auf
  → Erzeugt: 02-stages/99-logs/CSV00-root.resolved.txt
  → Bei Fehler: Abbruch mit Fehlermeldung

Schritt 2: Modell-Inventar erstellen
  python CSV01-validate_model.py
  → Scannt 00-model/00-archimate/ nach .archimate Dateien
  → Erzeugt: 02-stages/model-scope.txt
  → Nur informativ, trifft keine Entscheidungen

Schritt 3: Scope auflösen
  python CSV03-resolve_run_scope.py
  → Liest csvmapping.txt (01-artifacts/02-csv/01-mapping/)
  → Matched Modelle aus model-scope.txt gegen Mapping-Regeln
  → Erzeugt: 02-stages/run-scope.txt mit SOURCE=archi Einträgen
  → OEF, XLSX und CSV Quellen werden erst in Schritt 4 ergänzt
...
Schritt 6: Archi-Child-CSVs anhängen
  ...
  → Liest aus: 01-artifacts/02-csv/03-child/00-archimatechild/
...
Schritt 7: XLSX konvertieren und anhängen
  ...
  → Mapping via 01-artifacts/03-XLSX/01-mapping/csvmapping.txt
...
Schritt 9: master.xml in CSV konvertieren
  ...
  → Liest aus: 01-artifacts/00-xml/00-master/master.xml
  → Dispatcher: sourceSystem="archi" → OVERWRITE, "bpmn" → APPEND
  → BPMN-Mapping via 01-artifacts/00-xml/01-mapping/bpmnmastercsvsync.txt
  → Mapping gilt ausschließlich für den BPMN-Pfad — ArchiMate wird
    direkt aus dem XML-Namespace geparst, kein externes Mapping erforderlich

Schritt 10: Quality Gate (bei Bedarf)
  python CSV98-clean_master.py
  → Steht als Quality Gate am Ende des Flows zur Verfügung
  → Wird nicht bei jedem Lauf ausgeführt — nur bei Bedarf
  → FIX-01: Formula-Prefix (="...") wird entfernt
  → FIX-02: Backtick/Accent ´ wird zu Apostroph '
  → Idempotent — mehrfaches Ausführen ist sicher

Schritt 11: Export-Snapshot erstellen
  python CSV99-export_snapshot.py
  → Dedupliziert Master CSVs (last-wins)
  → Filtert Typen gemäß 01-artifacts/02-csv/02-sync/csvexport.txt
  → Schreibt nach: 01-artifacts/02-csv/04-import/
...
01-artifacts/02-csv/01-mapping/csvmapping.txt
  Format: "XLSX_Header","Sheet","CSV_Header","target.csv"
  Zweck: Steuert welche Modelle im Scope sind — wird von CSV03 gelesen

01-artifacts/03-XLSX/01-mapping/csvmapping.txt
  Format: "XLSX_Header","Sheet","CSV_Header","target.csv"
  Zweck: Steuert welche XLSX-Spalten in welche CSV-Spalten fließen (CSV07)

01-artifacts/03-XLSX/01-mapping/propmapping.txt
  Format: "XLSX_Header","Sheet","CSV_Header","properties.csv"
  Zweck: Mapping für Properties-Sheet (CSV08)

01-artifacts/02-csv/02-sync/csvexport.txt
...
ARCHI IMPORT
--------------------------------------------------------------------------------
Nach erfolgreichem CSV99 liegen in 01-artifacts/02-csv/04-import/ drei Dateien:
  elements.csv
  relations.csv
  properties.csv
...
================================================================================
CSV FLOW HOW2 | S105 | 2026-04-14 | R+MUNI Blueprint
================================================================================
